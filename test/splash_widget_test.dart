...
void main() {
  testWidgets('When Splash Screen Open', (WidgetTester tester) async {
    ...
    expect((image_widget_icon.image as AssetImage).assetName, 'assets/images/ic_high_style.png');

    Finder text_widget_welcome_message_finder = find.byKey(Key("text_widget_welcome_message"));
    expect(text_widget_welcome_message_finder, findsOneWidget);
    final text_widget_welcome_message = tester.widget<Text>(text_widget_welcome_message_finder);
    expect(text_widget_welcome_message.data,'Welcome to \n High Style Apparel');
    expect((text_widget_welcome_message.style as TextStyle).color, Colors.white);
    ...
    Finder outlinebutton_widget_login_finder = find.byKey(Key("outlinebutton_widget_login"));
    expect(outlinebutton_widget_login_finder, findsOneWidget);
    final outlinebutton_widget_login = tester.widget<OutlinedButton>(outlinebutton_widget_login_finder);
    expect(outlinebutton_widget_login.child, isA<Text>());
    expect((outlinebutton_widget_login.child as Text).data ,'LOG IN');
    expect(((outlinebutton_widget_login.child as Text).style as TextStyle).color ,Colors.white);

    Finder outlinebutton_widget_signup_finder = find.byKey(Key("outlinebutton_widget_signup"));
    expect(outlinebutton_widget_signup_finder, findsOneWidget);
    final outlinebutton_widget_signup = tester.widget<OutlinedButton>(outlinebutton_widget_signup_finder);
    expect(outlinebutton_widget_signup.child, isA<Text>());
    expect((outlinebutton_widget_signup.child as Text).data ,'SIGN UP');
    expect(((outlinebutton_widget_signup.child as Text).style as TextStyle).color ,Colors.white);
